...
export function WorkspaceNativeSessionView({
  ...
  }, [initialPrompt, seedMessages, session.runtime_id]);
  ...
  const isAwaitingResponse = isSending
    || isProcessingTurn;
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      await sendNativeSessionInput(session.runtime_id, text);
      await pollEvents();
      await refreshSummary();
    } catch (error) {
  ...
  }, [composerText, pollEvents, refreshSummary, sendNativeSessionInput, session.runtime_id, t]);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <WorkspaceSessionComposer
        ...
        disabled={isTerminalStatus(session.status) || hasAttentionPanel}
        canSubmit={canSend}
        isSubmitting={isSending}
        submitLabel={t('workspace.composeSend')}
        loadingLabel={t('common.loading')}
        onPrimaryAction={
          !isTerminalStatus(session.status) && isProcessingTurn
            ? () => void handleStop()
            : () => void handleSend()
        }
        primaryActionLabel={
        ...
        controls={(() => {
          ...
          return (
            <>
              <LiveProviderIcon provider={session.provider} />
